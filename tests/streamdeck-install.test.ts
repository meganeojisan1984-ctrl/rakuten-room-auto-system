...
test("parseKeysSpec: 既定は写真と同じ 3 キー", async () => {
  const { parseKeysSpec } = await importTool("install.mjs");
  const keys = parseKeysSpec(undefined);
  assert.deepEqual(
    keys.map((k: any) => `${k.position}:${k.provider}:${k.window}`),
    ["0,0:codex:weekly", "1,0:claude:weekly", "2,0:claude:5h"]
  );
});
...
test("parseKeysSpec: 指定文字列を解釈し、不正値は弾く", async () => {
  ...
  assert.equal(keys[1].position, "1,0");
