...
まず診断コマンドを実行してください。導入状態・アプリのバージョン・キーの配置・ログ・データ取得をまとめて確認できます。

```bash
npm run streamdeck:doctor
```
...
| キーの見え方 | 原因 | 対処 |
|---|---|---|
| 真っ黒・空白のまま | プラグインが読み込まれていない（アクションの UUID が未知の状態） | Stream Deck が 6.5 以上か確認 → `npm run streamdeck:install` で再導入 → アプリを再起動 |
| メーターのアイコンだけ出る | プラグインは動いているが画像の描画が届いていない | キーを 1 回押して再取得。直らなければ `npm run streamdeck:doctor` のログ欄を確認 |
| `--%` と「取得失敗」 | プラグインは正常。データ取得だけ失敗している | 下の「うまく取得できないとき」を参照 |
| 右パネルに「AI 使用量」が出てこない | プラグインが未導入、またはアプリのバージョン不足 | `npm run streamdeck:doctor` の 1〜2 を確認 |
