...
function buildPrompt(item: RakutenItem): string {
  ...
  return `あなたはInstagramとXで多くのフォロワーを持つ楽天ROOMのインフルエンサーです。以下の商品情報をもとに、SNSで話題になりそうな楽天ROOM投稿文を日本語で生成してください。
...
【生成ルール】
1. InstagramやXでバズりやすい「共感・驚き・憧れ」を引き出す書き方にしてください
2. 「この商品を使うと生活・見た目・気分がどう変わるか」というベネフィットを強調してください
3. お得情報（ポイントアップ、クーポン）がある場合は必ず冒頭で最優先にアピールしてください
4. 自然なインフルエンサー口調で、広告感・自動投稿感を出さないでください
5. 全体で150〜250文字程度（ハッシュタグ含む）に収めてください
6. 末尾にSNSで検索される関連ハッシュタグを4〜6個付けてください（#楽天ROOM #買ってよかった を必ず含める）
7. 絵文字を効果的に使って視認性を高めてください
