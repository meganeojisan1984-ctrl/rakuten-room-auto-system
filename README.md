...
Instagramカルーセルは、商品情報を分析してから「悩み→商品に合った理解ポイント→使う場面→価格・レビュー→楽天ROOM導線」の5枚を企画します。②は、サイズ商品なら寸法、美容品ならケア場面、キャンプ用品なら設営・携帯性などに切り替わります。各スライドには商品別の見出し・補足文・手書き風線・カード・矢印などの文字設計を作り、AIには背景の視線誘導だけを生成させ、正確な日本語文字は一度だけ合成します。`AI_IMAGE_ENABLED=1` かつ `OPENAI_API_KEY` が設定されている場合は、企画に沿った5枚分の背景画像を生成します。OpenAIの残高不足や一時的な生成失敗時は、商品別の文字と装飾を保持した決定論的カルーセルへ自動フォールバックします。
