...
Instagramカルーセルは、商品情報を分析してから「悩み→商品に合った理解ポイント→使う場面→価格・レビュー→楽天ROOM導線」の5枚を企画します。②は、サイズ商品なら寸法、美容品ならケア場面、キャンプ用品なら設営・携帯性などに切り替わります。各スライドには商品別の見出し・補足・手書き風線・カード・矢印などの文字設計を作ります。`AI_IMAGE_ENABLED=1` かつ `OPENAI_API_KEY` が設定されている場合は、`docs/creative/references`（または `AI_IMAGE_REFERENCE_DIR`）の参考画像、楽天商品画像、商品別の5ページプロンプトを使って、文字入りの完成画像を5枚生成します。`AI_IMAGE_TEXT_MODE=0` にすると従来の背景生成＋正確な文字合成へ切り替えられます。OpenAIの残高不足や一時的な生成失敗時は、商品別の文字と装飾を保持した決定論的カルーセルへ自動フォールバックします。

OpenAIのテキスト生成が残高不足の場合も、リトライで時間を消費せず、商品ブリーフに基づく紹介文へ切り替えます。2026-09-25はROOM投稿自体は成功していましたが、OpenAIの `credit_balance_exhausted` によりInstagram画像生成と一部の紹介文生成がフォールバックしていました。クレジットを補充した後は、次回実行から通常のAI経路に戻ります。
...
`SETUP-INSTAGRAM-COMMENTS.md` に従ってMeta Webhooksを設定すると、コメントへの公開返信と、キーワード一致時の非公開返信を自動化できます。フォローだけを起点にした自動DMは公式APIの制約上対象外です。返信機能は安全のため `IG_COMMENT_AUTOREPLY_ENABLED=0` が既定です。
